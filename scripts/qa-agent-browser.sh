...
ab "navigate to a native message fragment" eval "(() => { const link = [...document.querySelectorAll('.session-toc .toc-assistant')].findLast((item) => { const href = item.getAttribute('href'); return href && new URL(href, location.href).pathname === location.pathname && new URL(href, location.href).hash; }); if (!link) throw new Error('Native history QA requires a recorded assistant anchor'); const anchor = decodeURIComponent(new URL(link.getAttribute('href'), location.href).hash.slice(1)); window.__qaNativeHistory = { href: location.href, entry: history.state.readerEntry, scrollY, focus: document.activeElement, anchor, target: null }; location.hash = '#' + encodeURIComponent(anchor); })()" >/dev/null
ab "wait for native fragment adoption and positioning" wait --fn "(() => { const state = window.__qaNativeHistory; state.target = document.getElementById(state.anchor); return Number.isInteger(history.state?.readerEntry) && history.state.readerEntry !== state.entry && state.target && document.activeElement === state.target && state.target.getBoundingClientRect().top >= 0 && state.target.getBoundingClientRect().top < innerHeight; })()" >/dev/null
...
if [[ "$completed_work_count" == "1" ]]; then
  ...
  assert_contains "completed work default disclosure" "$completed_work_closed" "true"
  ab "reveal completed work disclosure" scrollintoview "#tab-work [data-runtime-completed-work] > summary" >/dev/null
  ab "focus completed work disclosure" focus "#tab-work [data-runtime-completed-work] > summary" >/dev/null
  ab "expand completed work with keyboard" press Enter >/dev/null
