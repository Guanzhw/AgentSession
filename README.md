...
派发、跟进和返回节点保留在正文原位置；节点下方的局部连接可跳转到同一任务的上一或下一条关键记录，按记录顺序连接。普通协作消息归入所属执行过程，展开或查看来源仍可准确定位。“任务与协作”侧栏按需打开，先显示紧凑任务关系，再显示所选任务的最初请求和最近回复摘录；往来记录和完整子历史均可展开。模型与 token 统计收入每条消息的“用量与模型”，工具和推理仍可独立展开。
