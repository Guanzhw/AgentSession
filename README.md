...
和用户级的 Skills、Agents、Commands、Plugins、Hooks、Tools、Rules 或
Provider 扩展包。扩展类型、搜索路径和优先级仍由各 Provider 自己负责。
...
`extension_*` 用于查询选中的 Agent 运行时扩展；`artifact_*` 用于查询有界的
文件快照。这里的“扩展”与工件文件名后缀筛选是两个独立概念。
...
原工件后续发生变化，分析证据仍可审查。
`fileExtensions` 仅用于筛选这些工件目录中的文件名后缀；已有配置中的旧字段
`extensions` 仍继续兼容。
...
使用的工件根目录、显式文件和后缀筛选。点击 **恢复默认值** 时，会尽可能删除
Provider 专用差异，使该值重新继承 `analysis.targets` 或内置目标。
