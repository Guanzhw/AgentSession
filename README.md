...
派发、跟进和返回节点保留在正文原位置，局部连接可跳转到同一任务的上一或下一条关键记录。普通协作消息归入所属执行过程，展开后可读完整内容。
“任务与协作”侧栏先用有方向的局部关系图呈现分派和结果返回，选择任务或连线后阅读逐次往来。图每组展示最多四条参与者与任务的关系；可搜索、分页的任务目录覆盖全部任务。同一子历史的多次运行归入一个任务，运行和消息分别续读，最初请求、最近回复与完整子历史均可展开。按需打开的活动时间窗另用于查看同时发生的工作。模型与 token 统计收入每条消息的“用量与模型”，工具和推理仍可独立展开。
