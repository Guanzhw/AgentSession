...
OpenSessionViewer 基于 [OpenSession](https://github.com/HeavyBunny19C/OpenSession) 开发。这个项目保留了本地、多 Provider AI 会话浏览器的核心方向，同时迁移到 TypeScript，并继续推进更丰富的嵌套会话、调用链和上下文可视化。
...
- **Table of Contents**：长会话自动生成导航，定位用户提示、assistant turn、task 和 nested branch。
...
- recursive session tree、TOC、Flow 视图
- OpenCode 管理操作入口
...
3. **Context View**
   - 新增上下文视图，展示每个 session 的每个 step 实际放入了哪些 context。
   - 目标是回答：模型在某一步看到了哪些消息、文件、工具结果、摘要、系统提示、agent prompt 或压缩后的历史。

4. **Table of Contents Upgrade**
   - 继续增强长会话导航，覆盖 user prompts、assistant turns、tool calls、subagents、重要 milestone。
