...
设置页面会编辑 `analysis.providers.opencode.targets.<target>` 覆盖。每个目标
会明确显示默认使用的 Provider 无关分析材料根目录、显式文件和后缀筛选；
OpenCode 运行时上下文会在启动时自动解析。点击 **恢复默认值** 时，会尽可能
删除 OpenCode 专用差异，使该值重新继承 `analysis.targets` 或内置目标。
...
可以在 `analysis.providers.opencode.targets.<target>` 下覆盖 OpenCode 的目标
