...
启动前，会话页面还会按 Provider 解析当前本地运行时扩展，并允许选择项目级
和用户级的指令、Skills、Agents、Commands、Plugins、Hooks、Tools、Rules
或 Provider 扩展包。扩展类型、搜索路径和优先级仍由各 Provider 自己负责。
大多数 Provider transcript 不包含不可变的历史扩展清单，因此该选择器明确
表示“当前本地解析结果”，不会声称精确还原会话开始时加载的环境。每个捕获的
工件都会记录其对应的运行时扩展 ID。
...
`extension_*` 用于查询选中的 Provider 运行时上下文；`artifact_*` 用于查询
由已配置分析材料和所选可采集运行时扩展共同生成的有界快照。
...
`--disable-terminal-launch` 关闭；分析仍需要为每个 Provider 单独启用和配置：
...
- `skills`：所选 Provider 运行时 Skills
- `prompts`：提示词文件与模板
- `agents`：所选 Provider 运行时 Agent 定义与角色
- `docs`：文档目录
- `rules`：所选 Provider 运行时指令与规则
...
Provider 设置页面会编辑
`analysis.providers.<provider>.defaultTargets` 和
`analysis.providers.<provider>.targets.<target>` 覆盖。每个目标会明确显示默认
使用的 Provider 无关分析材料根目录、显式文件和后缀筛选；Provider 运行时
上下文会在会话页单独显示。点击 **恢复默认值** 时，会尽可能删除 Provider
专用差异，使该值重新继承 `analysis.targets` 或内置目标。
...
Provider 的目标配置，包括命令、提示词、shell、工件目录和文件后缀筛选。
其他自定义目标也可复用相同结构。
