import { buildOpenCodeSessionContainer, type SessionContainer } from "./session-container.js";
import { buildOpenCodeSessionContext, type SessionContextView } from "./context.js";
...
export function buildOpenCodeSessionMetrics(sessionId: string, dbPath = undefined): SessionMetricsView | null {
  const container = buildOpenCodeSessionContainer(sessionId, dbPath);
  if (!container) {
    return null;
  }

  const context = buildOpenCodeSessionContext(sessionId, dbPath);
