...
2. **OpenClaw current SQLite + native v3 core** — ✅ **已完成（2026-09-09）**。官方 release `v2026.9.3` commit `1391f7cd2d40ab5bbcf2f5f831d3a64f520e72d7`、审计 moving HEAD `0140d656b1012a3059fd8f769952485a58b8a4e5`，agent schema 19 的 SQL sha256 为 `fe93217454642e911608f81afc53c9fb3bb7c20cc32bc73f8f6eeaaf232b91b8`。现有只读 SQLite 读取保持 `session_nodes` canonical key、`session_windows` 代数和 `transcript_events` 原始事件；新增 bounded `session_nodes.entry_json` facts normalization 与 provider-owned native v3 core：所有非 header 记录的 canonical v2 event/branch refs、common fork ancestor、recorded Goal/Actor/owner、explicit spawn AgentRun/coordination、request Usage，以及有可读结果的 compact/branch summary Context evidence。决策记录：`.agents/decisions/implemented/2026-09-09-openclaw-native-v3-core.md`。限制：本机安装 2026.7.1-2 为 pre-flip、无当前格式数据目录，未升级本机 OpenClaw，live current-format 验证仍未完成；高级 SQLite evidence tables 保持后续独立范围。
