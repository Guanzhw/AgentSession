...
| DeepSeek Harness (alpha.2 snapshot) | Native events include goal change, workflow, and team tasks; team `blockedBy` can become dependencies. The official alpha.2 source establishes these mappings; local v0/v1 fixtures and minimal v2 fixtures keep the evidence bounded. | Native turn/step/tool lifecycle gives recorded execution evidence; v2 `assistant/attempt` is diagnostic only and never creates an assistant message. Background, async, and scheduled execution remain unknown without recorded events. | Parent/descriptor lineage, inbox splice, delivery events, workflow, and team message vocabulary are recorded. Current generation selection preserves one canonical root and keeps dangling children explicit. | Recorded compaction lifecycle and request context are present. v0/v1 `seedLength` and v2 `isSeeded` plus tagged `session/end-seed` establish inherited boundaries. No live memory/dream/experience evidence exists. | Assistant usage is selected from `data.usage` or the final embedded stream usage and folded last-wins by turn/step, with retry boundaries opening a new slot; empty assistant messages still count. Direct/inherited/shared request-origin attribution remains unknown without provider evidence. |
...
| Pi | Session files expose no Task or AgentRun abstraction. | No execution mode, attempt, or scheduler abstraction is recorded. | `parentSession` is lineage only and cannot distinguish rotation from explicit fork; it must not imply subagent work. | In-file branches, compaction, and branch summaries are recorded. Compaction becomes a metadata-only summary artifact. No memory/dream/experience evidence is present. | Pi billed totals include all recorded assistant usage, including abandoned/history branches; retainedTail copies are not counted again. No inherited/shared ownership evidence exists. |
...
OpenClaw current SQLite, Pi 0.84.4, Claude Code 2.1.207, OpenCode 1.17.11,
