...
**Status:** branch implementation and local release acceptance complete. The
five-provider and search decisions are implemented in this branch. Version
2.0.0 has not been released.
...
These are branch implementation and local acceptance findings. Publication is
...
| MCP hierarchical retrieval | Implemented; compiled and locally packed stdio protocol acceptance passed | `session_browse` traverses providers, indexed project directories, and source-checked session summaries. `session_search` can scope title, directory, user, or assistant matches and root/child lineage; metadata-only queries skip the transcript scan. `session_get` reports role/tool-name facets, and `session_timeline` locates message or tool-name events within a session. Legacy and `2026-07-28` clients reached all five available real providers. The installed local 2.0.0 tarballs exposed all six tools and browsed all five providers. Isolated cold connection/index refresh took 34.9 and 25.7 seconds; most calls took 1–7 ms, but one OpenCode browse took 26.5 seconds before a 2 ms repeat, so latency remains a follow-up. |
...
In a separate warm Codex navigation, a `MutationObserver` followed by a browser
frame observed the first visible conversation body at 1,857 ms; DOM ready was
3,148 ms, with 40 message groups initially rendered. This is a single local
sample, not a latency guarantee.
...
- `npm run ci:quality` passed with 1,060 tests after the MCP child-source
  ...
  rerunning with the normal startup mode passed. The final E2E included a
  native fragment in an initially
...
- A real Codex `README advertises` query against the final Viewer API returned
  ...
  had canonical source-event refs, snippets within 160 characters, and both
  terms visible. No transcript text was written to the QA log.
- On the packed 2.0 MCP, `session_search` with `providers: ["openclaw"]` and
  `session_get` with `provider: "hermes"` both return explicit schema errors
  listing the five accepted IDs. The packed Viewer rejects `--openclaw-dir`
  with an unsupported-provider error. The saved MCP launcher points to
  `@acetamido/agentsession-mcp@latest`; the registry still serves v1.10.1, so
  the saved launcher will not resolve v2 until 2.0 is published.
- The packed MCP returned reciprocal source-backed parent/child refs for a
  recorded OpenCode pair, a Codex pair, and a DeepSeek Harness pair. For each,
  `session_get` on both ends, `session_timeline` on both ends, and a title
  `session_search` kept canonical provider/session and event references. The
  local OpenCode scan found 73/73 resolvable parent links, Codex 498/498, and
  DeepSeek Harness 8/8. Local Claude Code (11 sessions) and Pi (one session)
  had no parented sample, so positive lineage behavior for those providers is
  not claimed. The sanitized local call log is
...
- On the isolated live Viewer, content-search results for all five providers
  ...
  `...84e4b`, Codex `...1ca583`, Pi `...466e`, and DeepSeek Harness
...
- A later consistent snapshot of the real Viewer database was made through
  SQLite's online backup API and refreshed with the built 2.0 service using
  only the snapshot as `AGENTSESSION_META_PATH`. All five provider refreshes
  returned `ok` in 22.3 seconds. The one OpenClaw and four Hermes index rows
  retained identical identity and full-row SHA-256 digests before and after;
  `session_meta` and all three token-stats tables retained their row counts,
  row digests, and schema digests. The supported Codex index gained four rows,
  so the full derived index changed from 990 to 994 rows as permitted. The
  original database was opened read-only, passed `quick_check`, and had
  unchanged size, modification time, and journal-sidecar state during the
  snapshot. A direct source-file hash was unavailable because another Windows
  process held an incompatible handle. The isolated snapshot was deleted;
  only sanitized comparison and diagnostics remain under
  `tmp/v2-qa/retired-preservation-20260926-acceptance/`.
...
one streaming pass, without changing languages. One packed-MCP run took 24.3
...
- [Five-provider decision](../../.agents/decisions/implemented/2026-09-26-five-provider-v2-scope.md)
- [Search-boundary decision](../../.agents/decisions/implemented/2026-09-26-complete-search-boundaries.md)
