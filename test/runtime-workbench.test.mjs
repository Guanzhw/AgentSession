...
import { formatLocalizedDurationMs, renderProgressiveContent } from "../dist/src/views/components.js";
...
test("reader and secondary disclosures stay balanced when reasoning contains replacement tokens", () => {
  ...
  const reasoningStart = main.indexOf('<div class="reasoning-body markdown progressive"');
  ...
  assert.match(main, /data-progressive-part-id="reasoning-1" data-progressive-field="reasoning"/);
  assert.doesNotMatch(main, /context context/);
  const content = tree.messages[0].parts[0].data.text;
  const first = renderProgressiveContent(content, "markdown", 0, 6000);
  assert.match(first.html, /context context/);
  assert.ok(first.html.includes("$`"), "replacement token remains literal in the loaded first chunk");
  assert.doesNotMatch(first.html, /data-reader-pane|id="tab-work"|id="tab-events"/, "content must not duplicate the surrounding reader structure");
  assert.equal((first.html.match(/<div\b/g) || []).length, (first.html.match(/<\/div>/g) || []).length);
  assert.equal((first.html.match(/<p>/g) || []).length, (first.html.match(/<\/p>/g) || []).length);
  assert.ok(first.nextOffset > content.indexOf("$`") && first.nextOffset < content.length);
  const last = renderProgressiveContent(content, "markdown", first.nextOffset, 6000);
  assert.match(last.html, /tail tail/);
